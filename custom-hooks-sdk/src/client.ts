 ...
 */
export class PermisoCustomHooksError extends Error {
  constructor(
    message: string,
...
export class PermisoCustomHooksClient {
  ...
  async sendEvent(
    ...
    if (!response.ok) {
      throw new PermisoCustomHooksError(
        `Custom Hooks API error: ${response.status} ${response.statusText}`,
        response.status,
        rawBody,
      );
    }

    let parsed: CustomHooksResponse;
    try {
      parsed = rawBody ? (JSON.parse(rawBody) as CustomHooksResponse) : {};
    } catch {
      throw new PermisoCustomHooksError(
   ...
   */
  async endSession(): Promise<CustomHooksResponse> {
